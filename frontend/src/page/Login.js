...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (email && password) {
      const fetchData = await fetch(`${process.env.REACT_APP_SEREVR_DOMAIN}/login`, {
        method: "POST",
        headers: {
    ...
    <div className='p-3 md:p-4'>
      <div className='w-full max-w-sm bg-white m-auto flex flex-col p-4'>
        ...
        <form className='w-full py-3 flex flex-col' onSubmit={handleSubmit}>
          ...
          <button className='max-w-[120px] w-full m-auto cursor-pointer bg-red-500 hover:bg-red-600 text-white text-xl font-medium text-center py-1 rounded-full mt-4'>Sign in</button>
        </form>
